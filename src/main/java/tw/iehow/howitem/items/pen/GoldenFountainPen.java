...
public class GoldenFountainPen extends BaseHowItem {

    public GoldenFountainPen() {
        super(Items.MOJANG_BANNER_PATTERN, 1337021);
    }

    private static void penShoot(PlayerEntity player, int range, boolean advanced, int particleCount) {
        ...
        for (int i = 0; i < range; i++) {
            ...
            player.getWorld().getEntitiesByType(
                ...
                entity.damage((ServerWorld) player.getWorld(), player.getDamageSources().mobProjectile(player, player), (float) 6.0);
